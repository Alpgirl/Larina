...
class LIFO {
    ...
    public: 
        ...
        void push(int value) {
            stek *MyStack = new stek(value);                // объявляем новую динамическую переменную типа stek + записываем значение
            MyStack->next = T;                              // связываем новый элемент стека с предыдущим
            T = MyStack;                                    // новый элемент стека становится его вершиной
        }
        void pop() {
            int temp = T->value;                            // извлекаем в переменную temp значение в вершине стека
            stek *MyStack = T;                              // запоминаем указатель на вершину стека, чтобы затем 
